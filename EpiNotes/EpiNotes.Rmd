...
Probabilities in the same probability space are independent if $$P(A) \perp P(B)  P(A \cap B) = P(A).P(B)$$
...
For continuous probability, the probability density function PDF is a continuous function, and although the probability of a single specific number is 0 the area under which still integrates to 1; integrating the pdf yields to cumulative distribution function CDF. The area under a part of the pdf corresponds to the total probability for those values of that random variable. 

Integrating the PDF yields the CDF
Cumulative Distribution Function is the probability that a random variable takes the value $\leq x$
	$F(x) = P(X \leqslant x)$
...
Remember the diagnostic likelihood ratio of a positive test (DLR+ve)
