...
knitr::kable(contingency, booktabs = TRUE, caption= "Contingency table", escape=FALSE)
...
This table is the familiar medical school one where sensitivity and specificity are shown to be mean completely different things about a positive test, depending on the prevalence of the disease in the population.
...
Generation time is latent plus infectious and serial interval is time between identical clinical moments in successive cases in the same chain of transmission. The corresponding – or not – clinical times are incubation and symptomatic. 
...
Vaccine efficacy is foolish, "usually" 1-(Risk in vaccinated/Risk in Unvaccinated). If due to reducing infectiousness it's VEi, if susceptibility Ves. 
...
Estimation depends on being able to see direct and indirect effects; with some assumptions VEs and VEi can be jointly estimated from only the disease rate in vacc v unvacc if the vacc proportion is varied in strata, clusters or times.  This joint estimation has a coverage estimate trading $VE_i$ against $VE_s$	and if a 2nd stage cluster, infection probability beta, $VE_s =0$, proportion infected at time t It, contact probability c, duration of contact d then in population of n individuals the hazard rate for infection if vaccinated is $\lambda_{0i}(t) = {c \beta I_{i}t}{n_i}$ and for unvaccinated is that times $1-VE_i$. 
...
# Model fit 
All models essemtially estimate a function *f* relating observations to an outcome. This estimated function $\hat{f}$ can be tested in various ways. Cross validation is a powerful way. 
