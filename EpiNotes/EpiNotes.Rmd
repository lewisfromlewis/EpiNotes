...
2. If a set of variables Z blocks all back door paths from exposure X to outcome Y, then simply observing them is sufficient to estimate their causal effect. The observations can be treated as intervention, $P(Y | do(X), Z) = P(Y| X , Z)$. 

3. If there are no causal paths from X to Y through Z, then $P(Y|do(X)) = P(Y)$. This can also be stated as ignoring the variables Z if $(Y \perp Z|W,X)$ in $G_{\bar X \bar{W(Z)}}$, the graph with arrows removed that go into X and into all W that are descendant of Z. 
