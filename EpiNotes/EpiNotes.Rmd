...
#render_book("EpiNotes.html", "Epinotes.epub", output_format = epub_book(pandoc_args = "--mathml", epub_version="epub3"))
...
The population is a sort of a holy quantity, those who could be cases: so case definition influences the definition of a population. Other influences are geographic, demographic and behavioural.

Population at risk is the entire actual population to which the case definitions are applied. Above that somewhere is a sort of superpopulation, an ideal version of the realised population.

## Measures of association

Relative risk is $r_e / r_u$ 
...
DEFF on total N is $1+(\bar{m} - 1) \rho$ where m is number in a cluster or, if cluster sizes vary, $1+ \rho(\=m(1 +(\frac{sd_m}{\=m})^2) - 1)$ where $\frac{sd_m}{\bar m}$ is coefficient of variation of cluster size
...
If testing Ha: μ > μ0, $1 - \beta = P (\=X > \mu_{0} +z_{1-\alpha} \frac{\sigma}{\sqrt{n}}; \mu = \mu_{a})$ , where $\bar X \sim N(\mu_a, \sigma^2/n)$ and μ0, α are known.  If 3 of the 4 unknowns μa, σ, n, β are specified the last can be calculated
	
If assuming only the noncentral t distribution then power is $P = (\frac {\=X - \mu_{0}} {s / \sqrt{n}} > t_{1-\alpha, n-1}; \mu = \mu_{a})$ ; note P=α at μa=μ0 ;
...
The R^2^ is $\frac {Regression SS}{Total SS}$, giving a proportion of variance explained by the regression or grouping.
	...
	As $Var(X) = \frac{1}{n -1} \sum_{i=1}^n (X_i - \=X)^2$, so 
	$Cov(X, Y) = \frac{1}{n -1} \sum_{i=1}^n (X_i - \=X)(Y_i - \=Y) = \frac{1}{n-1}  \sum_{i=1}^n (X_i Y_i - n \=X \=Y)$ and
...
For $Y_i = \beta_0 + \beta_1 X_i$ the sum of squares = $\sum_{i=1}^n (Y_i - \beta_0 + \beta_1 X_i)^2$ is minimised
	when $\beta_1 = 0 , \hat \beta_0 = \=Y$ and when $\beta_0 = 0 , \hat \beta_1 = \frac{\sum_{i=1}^n X_i Y_i}{\sum_{i=1}^n X_i^2}$

the estimator that minimises the sum of squares for a regression line is $\beta_1 = \frac{\sum_{i=1}^n (Y_i - \=Y)(X_i - \=X)}{\sum_{i=1}^n (X_i - \=X)^2}$
...
The numerator for total variability in response variable is $\sum_{i=1}^n (Y_i - \=Y)^2$ and is made up of the variability explained by the line and the residual variability.
