...
For causal inference, positivity is assumed. This is when the probability of a givenlevel of exposure, conditional on strata of risk and on all covariates, is greater than zero and less than one: 0<P(A=a|L)<1 for all a in A and L. 

Violations of this mean the causal effect can't be estimated without bias. Structural violations are common in single arm trials, or trying to apply information from individuals with 1 or 0 probability of the exposure to those with an intermediate probability. Random violations (no observations to compare simply by the play of chance) can be dealt with by interpolation from those on either side of the probability of receipt.
...
We want models to identify regular features (which occur in every sample, including future samples), but not to be influenced by irregular features (which are idiosyncratic, maybe represented as "error" or outliers or not repeated in future samples). Overfitting is when a model is so directed to the training set that it predicts test sets poorly, and underfitting is when the model fails to learn from features in the training set, and reflects both training and test sets poorly. 

Underfit models are insensitive to the sample: they are little changed by leaving one observation out. Overfit models change dramatically with exclusion of some observation. 

Bias-variance tradeoff isn't exactly the same distinction but is related, as bias to underfitting is kind of variance to overfitting.

One way of looking at models is as a form of encoding, a lossy compression. Generating predictions or more general estimates of relationships are thus like decompressing. The more parameters, the less this loss; yet learning from the data requires some compression. The [Minimum Description Length]() is one way to select a model. 
...
Out of sample accuracy is as good a judge of value of a predictive model as any, but no predictive metrics can say anything about whether a causal model is better than another. Predictive models can be made extremely accurate using no causal relationships at all, such as the use of race in predicting outcomes in a racist system. 
...
When two parameters are independent of each other, yet share a child, then conditioning on that child renders them conditionally dependent on each other. Coefficients which would be zero are rendered non zero when information about the child is added, because of the common cause. 
...
## Information criteria from Rethinking 
