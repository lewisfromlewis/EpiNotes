...
- Individual matching takes cases and only selects controls who possess the same value of the matching characteristic. Matched analysis is needed across strata j, such as for odds ratio $OR_m = \frac{\sum D_{1j} D^c_{0j} / N_j}{\sum D_{0j} D^c_{1j} / N_j} , ^{\times}_\div e^{1.96 \sqrt{\frac{1}{N_{01}} + \frac{1}{N_{10}}}}$, and tested with $\chi^2 = \frac{(n_{10} + n_{01})^2}{(n_{10} - n_{01})}, df = 1$
...
If the matched variable is on the causal pathway it underestimates the effect of exposure on outcome. 
