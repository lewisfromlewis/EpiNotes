...
The original invention of GAMs was with a local scoring algorithm to smooth the scatterplot. The modern random effects seems similar to PIRLS, penalised iterative least squares.

## Penalised likelihood 

The object of the GAM is to maximise the penalised likelihood function. There is [much detail](#loess) on what this function would be, centred around the degree of smoothing using a smoothing parameter. 

## Smoothing parameter 

There are various ways of choosing the smoothing parameter. General cross validation is one, or restricted maximum likelihood (REML) as the modern, parameters-as-distributions approach. 
