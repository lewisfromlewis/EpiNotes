...
k is estimated for power reasons from $\sigma c$ : see below or $m= \frac{N(1-\rho)}{(c - \rho N)}$ for cluster size given total N under simple random and number of clusters c. ICC can’t be used when the outcome is a rate per time; so k is used here. 
...
For two means:  		 
$n = \frac{4(z_{1 - \alpha/2} + z_{1-\beta})^2}{d^2}$, where $d = \frac{\delta}{\sigma}$,
or $\frac{(u+v)^2(\sigma_1^2 + \sigma_2^2)}{\delta^2}$
