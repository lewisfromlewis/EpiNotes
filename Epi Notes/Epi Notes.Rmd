...
Odds ratio is r/s, the ratio of discordance between the tests, and error factor is  so
	OR with 95% CI is  	to	 

the discordant proportion is  and its standard error is  

	so 95% CI for an estimate of the proportional discordance is 
...
Rate data
	The rate, call it λ, is estimated by , usually Events / Total time at risk 
	S.E. for the estimate is  so 95% CI is 
	Rate differences are 
		Expected number of events e1 is 
...
Assumes symmetrical distribution
	...
	there are   possible pairs of differences, each of which has an average difference.
		the real values of these averages are ordered  
		where T5 is the T with probability 0.025 (the 2-sided p value of 0.05 on the tables)
		 to  are the outer bounds of the 95% confidence interval for the difference

Wilcoxon Rank Sum for the difference of medians in two samples (Mann Whitney U / MWW)
...
take two samples of size N1 and N2, with a total size N
	...
	Ui is , and the total possible is 
		 for ROC curves
...
If appropriate to use a questionnaire, develop one that is easy to answer, process and analyse, minimises measurement error, preserves dignity and privacy of respondent and limits burden on respondents.  "The minimum amount of an individual's total experience that will provide sufficient information concerning the problem under study" (including exposure and covariates).
	...
	Responder bias is a type of selection bias, the responder odds ratio being biased unpredictably by the ratio of probabilities of selection from a contingenc table:
		 which is from 0.8 to 1.4 times the true OR
